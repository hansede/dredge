...
class TCGParser
  ...
  def self.parse_set(name, page)
    ...
    ActiveRecord::Base.transaction do
      rows.each do |row|
        name = scrub(row.css('td:first > font').text.downcase)
        cmc = scrub(row.css('td:eq(2) > font').text.downcase)
        type = scrub(row.css('td:eq(3) > font').text.downcase)
        color = scrub(row.css('td:eq(4) > font').text.downcase)
        rarity = scrub(row.css('td:eq(5) > font').text.downcase)
